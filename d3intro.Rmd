...
[API](https://github.com/d3/d3-selection/blob/v1.4.0/README.md#selection_attr)
...
[API](https://github.com/d3/d3-selection/blob/v1.4.0/README.md#selection_style)
...
1. **SVG `<text>`** -- This tag can be tricky.  It differs from HTML text tags (`<p>, <h1>, <h2>,` etc.) in that it has `x` and `y` attributes that allow you to position text on an SVG canvas.  Unlike HTML, the *fill* attribute controls the color of the text. Compare:
...
3. **`style`** is an attribute! To further complicat matters, `.style()` is just shorthand for `.attr("style", "...")` so the following are in fact equivalent:
