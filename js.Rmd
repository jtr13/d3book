...
Sorting in JavaScript sorts by character and modifies the original array^[If you don't wish for that to happen, you'll need to make a [deep copy](https://medium.com/javascript-in-plain-english/how-to-deep-copy-objects-and-arrays-in-javascript-7c911359b089){target="_blank"}.]:
...
Quantiles works a bit differently in D3: *the value returned depends on the sort order of x.* Therefore it is crucial to [sort the array](just-enough-js.html#sorting) first. Next, `p` takes a single value, not an array as in R.
